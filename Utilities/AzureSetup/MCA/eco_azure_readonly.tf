...
resource "null_resource" "billing_reader_role_assignment" {
  provisioner "local-exec" {
    command = 'az role assignment create --assignee ${azuread_application_registration.create_registered_app.id} --role "Billing Reader" --scope "/providers/Microsoft.Billing/billingAccounts/${var.billing_account_id}"'
  }
}
