...
def call_with_retry(func, *args, retries=5, initial_delay=2, backoff=2, **kwargs):
    """
    Calls func(*args, **kwargs), retrying on SpotinstClientException / ConnectTimeout
    with exponential backoff. Re-raises the last exception if all attempts fail.

    Useful for Spot API calls that have been observed to intermittently return
    403/errors on retry-safe (read-only or idempotent) operations even with a
    valid, unchanged token -- e.g. a backend node with a stale auth cache.
    """
    delay = initial_delay
    last_exception = None
    ...
    for attempt in range(1, retries + 1):
        ...
        except (SpotinstClientException, ConnectTimeout) as e:
            last_exception = e
            if attempt == retries:
                break
            log(
                f"Spot API call failed on attempt {attempt}/{retries} "
...
def get_or_create_spot_account(subscription_id, name, token):
    ...
    try:
        existing_accounts = [account["account_id"] for account in call_with_retry(client.get_accounts) if
    ...
    if not account_id:
        ...
        account_result = call_with_retry(client.create_account, name)
...
def run_command(cmd, *args):
    ...
    if run_in_shell:
        # subprocess.run(list, shell=True) is NOT portable: on Windows the list
        # is joined into a full command line, but on POSIX (macOS/Linux) only
        # cmd_to_run[0] is actually executed as the shell command and the rest
        # become ignored positional shell parameters. Build an explicit,
        # correctly-quoted command string instead so behavior is consistent
        # across platforms.
        if platform.system() == "Windows":
            command_to_execute = subprocess.list2cmdline(cmd_to_run)
        else:
            command_to_execute = shlex.join(cmd_to_run)
    else:
        command_to_execute = cmd_to_run

    run_process_result = subprocess.run(
        command_to_execute, stdout=subprocess.PIPE, stderr=subprocess.PIPE, encoding="utf-8", shell=run_in_shell
...
def build_custom_role(custom_role_name, custom_role_json_local_path, subscription):
    ...
    if custom_role_json_local_path is not None:
        with open(custom_role_json_local_path) as json_file:
            role_definition = json.load(json_file)
    else:
        ...
        if get_custom_role_from_s3_response.status_code in HTTP_OK_RESPONSE_STATUSES:
            role_definition = get_custom_role_from_s3_response.json()
        else:
            raise Exception(
                f"Failed to retrieve custom role JSON from S3. "
                f"Status code: {get_custom_role_from_s3_response.status_code}"
            )

    # Round-trip through json.dumps/json.loads (instead of str()) so the
    # placeholder substitution below operates on valid JSON (double-quoted)
    # rather than a Python dict repr (single-quoted), and the result stays
    # a proper dict that can be json.dump()'d later.
    role_definition_str = (
    ...
    return json.loads(role_definition_str)
...
def assign_roles(app_registration_id, roles, subscription):
    ...
    for role in roles:
        ...
        # A newly-created custom role isn't always immediately visible to the
        # role-assignment API across Azure's directory replicas. Retry with
        # increasing backoff rather than a single fixed-delay retry, since
        # propagation has been observed to occasionally take longer than 15s.
        delays = [15, 30, 45, 60]
        last_exception = None
        for attempt, delay in enumerate([0] + delays, start=1):
            if delay:
                log(
                    f"Role assignment for '{role}' failed, likely due to role "
                    f"propagation latency. Waiting {delay}s before retrying "
...
def get_or_create_custom_role(custom_role_name, custom_role_json_local_path, subscription, _retry_count=0):
    """
    ...
    Args:
        ...
        _retry_count (int): Internal guard against infinite recursion on the
            RoleDefinitionWithSameNameExists fallback below. Do not pass explicitly.
    ...
    """
    # NOTE: custom role names are unique at the tenant/directory level in Azure,
    # not per-subscription. Checking with --scope here would miss a role that
    # was already created for a *different* subscription in the same tenant.
    # We still explicitly target `subscription` via --subscription rather than
    # relying on the Azure CLI's ambient default subscription context, since
    # that may not match the subscription this onboarding run is operating on.
    ...
    new_scope = f"/subscriptions/{subscription}"
    ...
    if not role_already_exists:
        ...
        # Sanity check / diagnostic: confirm the roleName placeholder substitution
        # actually took effect before sending this to Azure.
        sent_role_name = custom_role.get("properties", custom_role).get("roleName")
        log(f"Prepared custom role definition with roleName: {sent_role_name}")
        if sent_role_name != custom_role_name:
            log(
                f"WARNING: expected roleName '{custom_role_name}' but the built role "
                f"definition has roleName '{sent_role_name}' -- the CUSTOM_ROLE_NAME "
                f"placeholder may not match the template. Check the template's actual "
                f"placeholder token.",
                LogLevel.ERROR,
            )
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
            json.dump(custom_role, f)
            temp_file = f.name

        url = (
            f"https://management.azure.com/subscriptions/{subscription}"
            f"/providers/Microsoft.Authorization/roleDefinitions/{custom_role_id}"
        ...
        try:
            ...
            if "RoleDefinitionWithSameNameExists" in str(e) and _retry_count < 3:
                # race condition: role was created (e.g. by a concurrent run) between
                # our existence check and this create call. Fall back to extending
                # the existing role's assignableScopes instead of failing. Capped at
                # 3 retries so a persistent lookup mismatch can't recurse forever.
                log(
                    f"Custom role '{custom_role_name}' already exists (race with "
                    f"another onboarding run?). Falling back to updating its assignableScopes "
                    f"(attempt {_retry_count + 1}/3).",
                    LogLevel.INFO,
                )
                return get_or_create_custom_role(
                    custom_role_name, custom_role_json_local_path, subscription,
        ...
        existing_role = get_custom_role_response[0]
        result = existing_role["roleName"]
        log(f"Found already existing custom role: {result}")

        assignable_scopes = existing_role.get("assignableScopes", [])

        if new_scope not in assignable_scopes:
            # The role already exists (for other subscription(s)) but hasn't been
            # granted on this subscription yet -- extend its assignableScopes
            # rather than trying to create a second role with the same name
            # (which Azure rejects with RoleDefinitionWithSameNameExists).
            log(f"Adding subscription {subscription} to assignableScopes of existing custom role: {result}")

            existing_role_id = existing_role["name"]  # this is the role definition GUID
            updated_body = {
                "properties": {
                    "roleName": existing_role["roleName"],
                    "description": existing_role.get("description", ""),
                    "permissions": existing_role["permissions"],
                    "assignableScopes": assignable_scopes + [new_scope],
                }
            }

            with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
                json.dump(updated_body, f)
                temp_file = f.name

            update_url = (
                f"https://management.azure.com/subscriptions/{subscription}"
                f"/providers/Microsoft.Authorization/roleDefinitions/{existing_role_id}"
                f"?api-version=2022-04-01"
            )
            ...
            log(f"Updated assignableScopes for custom role: {result}")
