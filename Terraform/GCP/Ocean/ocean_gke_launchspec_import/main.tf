...
### Spot Ocean Launchspec resource - Dynamically create a lauchspec for each node-pool in GKE. ###
resource "spotinst_ocean_gke_launch_spec" "launchspec" {
  ...
  dynamic "metadata" {
    for_each = data.google_compute_instance_template.template[each.key].metadata
    content {
      key   = metadata.key
